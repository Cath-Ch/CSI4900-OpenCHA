...
class StressAnalysisBinary(Affect):
    """
    **Description:**
    ...
    name: str = "affect_stress_analysis_binary"
    chat_name: str = "AffectStressAnalysisBinary"
    description: str = (
        "When a request for analysis of hrv data is received to estimate the stress level (binary - stressed or not stressed), "
    ...
    outputs: List[str] = [
        "Returns 1 if the individual is stressed, 0 if the individual is not stressed. Convert the number into proper interpretation"
    ]
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            from openCHA.tasks.affect.PredictorBinary import PredictorBinary
            import torch

            values["AE"] = AE
            values["torch"] = torch
            values["Predictor"] = PredictorBinary
